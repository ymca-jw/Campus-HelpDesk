...
public class AnswerDAO {
    ...
    // 담당자가 작성한 답변을 DB에 등록하는 기능
    public void insertAnswer(AnswerDTO answer) {
        String sql = "INSERT INTO answers (complaint_id, staff_id, content) VALUES (?, ?, ?)";
    ...
    public AnswerDTO findByComplaintId(Long complaintId) {
        String sql = """
                SELECT a.answer_id, a.complaint_id, a.staff_id, u.name AS staff_name, a.content, a.created_at
                FROM answers a
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    dto.setStaffName(rs.getString("staff_name")); // 담당자 이름
    ...
    public void updateAnswer(AnswerDTO answer) {
        String sql = "UPDATE answers SET content = ? WHERE answer_id = ?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
